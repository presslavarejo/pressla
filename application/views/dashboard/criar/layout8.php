<script>
    ...
    function layout8(ctx,w,h,fundo,cartaz,logo=false,i=0){
        ...
        ctx.font = ""+(150*tamanho_fonte)+"px "+$('#fonte').val(); 
        ctx.fillText(produtol1, 50, 150 + (h/4), h-100);
        
        ctx.fillStyle = cor_produtol2 ? cor_produtol2 : "#000000";
        ctx.font = ""+(150*tamanho_fonte)+"px "+$('#fonte').val();
        ctx.fillText(produtol2, 50, 280 + (h/4), h-100);

        ctx.fillStyle = cor_produtol3 ? cor_produtol3 : "#000000";
        ctx.font = ""+(150*tamanho_fonte)+"px "+$('#fonte').val();
        ctx.fillText(produtol3, 50, 410 + (h/4), h-100);
        ...
        ctx.textAlign = "left";
        ctx.font = "bold 50px "+$('#fonte').val();
        ctx.fillText("R$", 2*w/3, h-350, 100);

        if($("#layout").val() == 10){
            ctx.font = "20px "+$('#fonte').val();
            ctx.fillText($("#acimade").val(), 2*w/3, h-115, w/3);
        } else if($("#layout").val() == 11){
            ctx.font = "50px "+$('#fonte').val();
            ctx.fillText(unidade, 2*w/3, h-85, w/3);
        }

        //INCLUE O PREÇO GRANDE
        ctx.textAlign = "center";
        ctx.font = "bold 215px "+$('#fonte').val();
        ctx.fillText(preco, 4*w/5, h-165, (w/3)-60);
        ...
        if(logo){
            if($("#incluilogo").prop("checked")){
                ...
                ctx.drawImage(logo, 50, h-100-altLogo, largLogo, altLogo);
            }
        }
